...
class _TreeCursor extends AbstractValueAccessor implements CauseCloseable, Cursor {
            ...
            if (tnode == null) {
                ...
                try {
                    if (mode == 0) {
                        ...
                        storeNoRedo(_LocalTransaction.BOGUS, value);
                    }
                } finally {
                    locker.unlock();
                }
                ...
                _LocalDatabase db = mTree.mDatabase;
                _LocalTransaction txn = db.threadLocalTransaction
     ...
     */
    private void runStoreTriggers(_Tree.TriggerNode tnode, byte[] value) throws IOException {
        if (tnode != null) {
            doRunStoreTriggers(tnode, value);
        }
    }
     ...
     * @param tnode not null
     */
    private void doRunTriggers(_Tree.TriggerNode tnode, byte[] value) throws IOException {
        do {
    ...
    final byte[] findAndStore(byte[] key, byte[] value) throws IOException {
        ...
        try {
            ...
            if (txn.lockMode() == LockMode.UNSAFE) {
                ...
                _Tree.TriggerNode tnode = mTree.mLastTrigger;
                if (tnode != null) {
                    find(txn, key, VARIANT_REGULAR, new _CursorFrame(), latchRootNode());
    ...
    private byte[] findAndStoreAutoCommit(byte[] key, byte[] value) throws IOException {
        try {
            ...
            if (mode == 1) {
                ...
                _Locker locker = mTree.lockExclusiveLocal(key, hash);
                try {
                    return doFindAndStore(mode == 0 ? null : _LocalTransaction.BOGUS, key, value);
                } finally {
                    locker.unlock();
                }
            } else {
            ...
            if (mode == 1) {
                ...
                try {
                    return doFindAndModify
                        (mode == 0 ? null : _LocalTransaction.BOGUS, key, oldValue, newValue);
                } finally {
                    locker.unlock();
                }
            } else {
     ...
     * @param tnode not null
     */
    private void doRunValueModifyTriggers(_Tree.TriggerNode tnode,
